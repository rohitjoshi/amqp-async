...
#undef LOG_OFF
   ...
   */
      ...
      inline static std::shared_ptr<spdlog::logger> &logger() {
          static std::shared_ptr<spdlog::logger> logger = spdlog::get("amqp_async_logger");
          return logger;
      }
      ...
      inline static std::shared_ptr<spdlog::logger> &event_logger() {
          static std::shared_ptr<spdlog::logger> logger = spdlog::get("amqp_async_event_logger");
          return logger;
      }
      ...
      static bool init(
          ...
          printf("Created a logfile: %s\n", details_logfile.c_str());
#ifdef DEBUG
             auto lightq_logger = spdlog::stdout_logger_mt("amqp_async_logger");
#else
             //auto lightq_logger = spdlog::stdout_logger_mt("amqp_async_logger");
             auto lightq_logger = spdlog::rotating_logger_mt("amqp_async_logger", details_logfile, 1048576 * 500, 10);
          ...
          auto lightq_event_logger = spdlog::rotating_logger_mt(
              "amqp_async_event_logger", event_logfile, 1048576 * 500,
              10);
          //spdlog::set_async_mode(1048576); //queue size
...
#define LOG_EMERG(msg, ...) \
        if(log_enabled && log::logger().get() && log::logger()->should_log(spdlog::level::emerg)) \
            log::log_write(log::LOG_EMERG, __FILE__, __LINE__, _FUNC_NAME_, msg, ## __VA_ARGS__);
/**
 * LOG_ALERT logging macro
 */
#define LOG_ALERT(msg, ...) \
        if(log_enabled && log::logger().get() && log::logger()->should_log(spdlog::level::alert)) \
...
*/
#define LOG_INFO(msg, ...) \
        if(log_enabled && log::logger().get() && log::logger()->should_log(spdlog::level::info)) \
            log::log_write(log::LOG_INFO, __FILE__, __LINE__, _FUNC_NAME_, msg, ## __VA_ARGS__);
...
#define LOG_DEBUG(msg, ...) \
        if(log_enabled && log::logger().get() && log::logger()->should_log(spdlog::level::debug)) \
...
#define LOG_TRACE(msg, ...) \
       if(log_enabled && log::logger().get() && log::logger()->should_log(spdlog::level::trace)) \
...
#define LOG_CUSTOM(type, file, line, msg, ...) \
        if(log_enabled && log::logger().get() && log::logger()->should_log((spdlog::level::level_enum)type)) \
